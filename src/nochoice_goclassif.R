...
server <- function(input, output, session) {
  ...
  observe({
    if(input$startgo != 0) {
      ...
      #manque un isolate qq part par la, le tableau n'est pas recalcule, mais il ne disparait pas
      ...
      output$enrichGo <- renderPlot({
        isolate ({ barplot(enrichDrop(), drop = TRUE, showCategory=input$gocat, title="Go Enrichment") })
      }) #goenrich
      output$enrichGodot <- renderPlot({ 
        isolate ({ dotplot(enrichDrop(), showCategory=input$gocat, title="GO Enrichment") })
      }) #goenrich
